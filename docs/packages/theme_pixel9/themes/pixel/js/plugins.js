...
window.PIXEL_accordionsInit = function( $accordionEl ){
				...
				$(this).toggleClass('accordion-active ' + elActiveClass, true).next().stop(true,true).slideDown("normal", function(){
					if( ( $('body').hasClass('device-sm') || $('body').hasClass('device-xs') ) && element.hasClass('scroll-on-open') ) {
						$('html,body').stop(true,true).animate({
							'scrollTop': clickTarget.offset().top - ( PIXEL.initialize.topScrollOffset() - 40 )
						}, 800, 'easeOutQuad' );
					}
				});
			} else {
...
window.PIXEL_portfolioAjaxloadInit = function(){
	...
	$('.portfolio-ajax .portfolio-item a.portfolio-ajax-trigger').off( 'click' ).on( 'click', function(e) {
		...
		if( !$(this).parents('.portfolio-item').hasClass('portfolio-active') ) {
			PIXEL_portfolioLoadItem(portPostId, prevPostPortId);
		}
		e.preventDefault();
	});
};

window.PIXEL_portfolionewNextPrev = function( portPostId ){
	let portNext = PIXEL_portfolioGetNextItem(portPostId);
	let portPrev = PIXEL_portfolioGetPrevItem(portPostId);
	$('#next-portfolio').attr('data-id', portNext);
	$('#prev-portfolio').attr('data-id', portPrev);
};

window.PIXEL_portfolioLoadItem = function( portPostId, prevPostPortId, getIt ){
	if(!getIt) { getIt = false; }
	let portNext = PIXEL_portfolioGetNextItem(portPostId);
	let portPrev = PIXEL_portfolioGetPrevItem(portPostId);
	if(getIt == false) {
		PIXEL_portfolioCloseItem();
		$portfolioAjaxLoader.fadeIn();
		let portfolioDataLoader = $('#' + portPostId).attr('data-loader');
		$portfolioDetailsContainer.load(portfolioDataLoader, { portid: portPostId, portnext: portNext, portprev: portPrev },
		function(){
			PIXEL_portfolioInitializeAjax(portPostId);
			PIXEL_portfolioOpenItem();
...
window.PIXEL_portfolioCloseItem = function(){
...
window.PIXEL_portfolioOpenItem = function(){
	...
	if( noOfImages > 0 ) {
		$portfolioDetails.find('img').on('load', function(){
			noLoaded++;
			let topOffsetScroll = PIXEL.initialize.topScrollOffset();
			if(noOfImages === noLoaded) {
				$portfolioDetailsContainer.css({ 'display': 'block' });
				$portfolioDetails.addClass('portfolio-ajax-opened');
				$portfolioAjaxLoader.fadeOut();
				setTimeout(function(){
					PIXEL.widget.loadFlexSlider();
					PIXEL.initialize.lightbox({ 'parent': $portfolioDetails });
					PIXEL.initialize.resizeVideos();
					PIXEL.widget.masonryThumbs();
...
window.PIXEL_portfolioGetNextItem = function( portPostId ){
	let portNext = '';
...
window.PIXEL_portfolioGetPrevItem = function( portPostId ){
	let portPrev = '';
...
window.PIXEL_portfolioInitializeAjax = function( portPostId ){
...
window.PIXEL_animationsInit = function( $animationEl ){
...
window.PIXEL_carouselInit = function( $carouselEl ){
	...
	$carouselEl.each( function(){
		...
		let carousel = element.owlCarousel({
			...
			onInitialized: function(){
				PIXEL.slider.sliderDimensions({ 'parent': element.parents('.slider-element') });
				PIXEL.initialize.lightbox({ 'parent': element });
				PIXEL.widget.hoverAnimation({ 'parent': element });
				PIXEL.widget.loadFlexSlider({ 'parent': element });
				PIXEL.widget.counter({ 'parent': element });
				PIXEL.widget.progress({ 'parent': element });
				PIXEL.initialize.resizeVideos();
...
window.PIXEL_cookieInit = function( $cookieEl ){
	...
	$('.gdpr-container').each( function(){
		...
		if( typeof getCookie !== 'undefined' && getCookie == '1' ) {
			...
			PIXEL.initialize.resizeVideos({ parent: element });
		} else {
...
window.PIXEL_countdownInit = function( $countdownEl ){
...
window.PIXEL_counterInit = function( $counterEl ){
	...
	$counterEl.each(function(){
		...
		if( element.hasClass('counter-instant') ) {
			PIXEL_runCounterInit( element, elCommaObj );
			return;
		}
...
window.PIXEL_runCounterInit = function( elCounter, elFormat ){
...
window.PIXEL_dataClassesInit = function( $dataClassesEl ){
...
window.PIXEL_dataHeightsInit = function( $dataHeightsEl ){
...
window.PIXEL_resizeVideosInit = function( $videoEl ){
...
window.PIXEL_gmapInit = function( $gmapEl ){
...
window.PIXEL_gridFilterInit = function( $gridFilterEl ){
...
window.PIXEL_hoverAnimationInit = function( $hoverAnimationEl ){
...
window.PIXEL_html5VideoInit = function( $html5Video ){
	...
	$html5Video.each(function(){
		...
		if( PIXEL.isMobile.any() && !element.hasClass('no-placeholder') ) {
...
window.PIXEL_gridContainerInit = function( $gridContainerEl ){
...
window.PIXEL_lazyLoadInit = function( $lazyLoadEl ){
...
window.PIXEL_lightboxInit = function( $lightboxEl ){
...
window.PIXEL_modalInit = function( $modalEl ){
...
window.PIXEL_linkScrollInit = function( $linkScrollEl ){
	...
	$linkScrollEl.each( function(){
		...
		if( linkElement.length > 0 ) {
			let linkElSettings = PIXEL_linkScrollSettings( linkElAnchor, $('[data-scrollto="'+ linkElAnchor +'"]') );
			setTimeout( function(){
...
window.PIXEL_linkScrollSettings = function( hash, element ) {
	...
	let settings	= {},
		...
		elOffset	= element.attr( 'data-offset' ) || PIXEL.initialize.topScrollOffset(),
...
window.PIXEL_masonryThumbsInit = function( $masonryThumbsEl ){
...
window.PIXEL_navtreeInit = function( $navTreeEl ){
...
window.PIXEL_notificationInit = function( $notifyEl ){
	...
	if( jQuery(elTarget).length > 0 ) {
		...
		jQuery(elTarget).get(0).addEventListener('hidden.bs.toast', () => {
			PIXEL_notificationInit( $notifyEl );
		});
	}
...
window.PIXEL_onePageModule = function( $onePageMenuEl ){
	...
	$onePageMenu.each( function(){
		...
		linkEls.each( function(){
			...
			if( linkElement.length > 0 ) {
				let linkElSettings = PIXEL_onePageSettings( linkElAnchor, $('a[data-href="'+ linkElAnchor +'"]') );
				setTimeout( function(){
...
window.PIXEL_onePageScroller = function(){
	$onePageMenu.find('[data-href]').parent().removeClass( onePageMenuActiveClass );
	$onePageMenu.find('[data-href="#' + PIXEL_onePageCurrentSection() + '"]').parent().addClass( onePageMenuActiveClass );
};

window.PIXEL_onePageCurrentSection = function(){
...
window.PIXEL_onePageSettings = function( hash, element ) {
	...
	if( !elOffset ) {
		element		= parent;
		elOffset	= parent.attr( 'data-offset' ) || PIXEL.initialize.topScrollOffset();
	}
...
window.PIXEL_pageTransitionInit = function( $wrapperEl ){
...
window.PIXEL_parallaxInit = function( $parallaxEl ){
...
window.PIXEL_roundedSkillInit = function( $roundedSkillEl ){
				...
				entries.forEach( function(entry) {
					if (entry.isIntersecting) {
						if (!element.hasClass('skills-animated')) {
							...
							PIXEL_runRoundedSkills( element, properties );
...
window.PIXEL_runRoundedSkills = function( element, properties ){
...
window.PIXEL_pricingSwitcherFn = function( checkbox, parent, pricing, defClass, actClass ) {
...
window.PIXEL_pricingSwitcherInit = function( $pricingSwitcherEl ){
	...
	$pricingSwitcherEl.each( function(){
		...
		PIXEL_pricingSwitcherFn( elCheck, elParent, elPricing, elDefClass, elActClass );
...
window.PIXEL_progressInit = function( $progressEl ){
...
window.PIXEL_quantityInit = function( $quantityEl ){
...
window.PIXEL_readmoreInit = function( $readmoreEl ){
	...
	$readmoreEl.each( function(){
		...
		if( elMaskD == 'true' ) {
			elMask.css({ 'height': elMaskSize, 'background-image': 'linear-gradient( '+ PIXEL_HEXtoRGBA( elMaskColor, 0 ) +', '+ PIXEL_HEXtoRGBA( elMaskColor, 1 ) +' )' });
		} else {
...
window.PIXEL_HEXtoRGBA = function( hex, op ){
	let c;
...
window.PIXEL_shapeDividerInit = function( $shapeDividerEl ){
...
window.PIXEL_stickySidebarInit = function( $stickySidebar ){
...
window.PIXEL_subscribeFormInit = function( $subscribeForm ){
				...
				$(form).ajaxSubmit({
					...
					success: function( data ) {
						...
						if( elAlert == 'inline' ) {
							...
							PIXEL.widget.notifications({ el: elResult });
						}
					}
				});
			}
...
window.PIXEL_swiperInit = function( $sliderEl ){
...
window.PIXEL_tabsInit = function( $tabsEl ){
	...
	$tabsEl.each( function(){
		...
		PIXEL_tabsResponsive( element );
		PIXEL_tabsResponsiveResizeInit( element );
...
window.PIXEL_tabsResponsive = function( $tabsEl ){
...
window.PIXEL_tabsResponsiveResizeInit = function( $tabsEl ){
	...
	$tabsEl.each( function(){
		...
		if( $body.hasClass('device-sm') || $body.hasClass('device-xs') ) {
			...
			PIXEL.widget.accordions({ 'parent': element });
...
window.PIXEL_textRotatorInit = function( $textRotatorEl ){
...
window.PIXEL_togglesInit = function( $toggleEl ){
...
window.PIXEL_youtubeBgVideoInit = function( $youtubeBgPlayerEl ){
